TRANSLATIONS = {
    'es': {  # Spanish (default)
        'yolo_on': 'YOLO: {0} det.',
        'yolo_off': 'YOLO: OFF',
        'det_power_label': 'DET.POWER',
        'det_power_short_low': 'L',
        'det_power_short_med': 'M',
        'det_power_short_high': 'H',
        'det_power_short_vhigh': 'VH',
        'tinysa_on': 'TinySA: ON',
        'tinysa_off': 'TinySA: OFF',
        'det_audio_on': 'DET AUDIO: ON',
        'det_audio_off': 'DET AUDIO: OFF',
        'audio_source_phone_mic': 'AUDIO: MIC MOVIL',
        'audio_source_esp32_array': 'AUDIO: ARRAY ESP32',
        'ip_label': 'IP: {0}',
        'ip_selector_title': 'Selector de IP',
        'ip_selector_prompt': 'Selecciona una IP disponible:',
        'ip_selector_no_candidates': 'No se encontraron IPs candidatas en Tailscale o red local.',
        'ip_selector_no_selection': 'Selecciona una IP de la lista.',
        'ip_selector_current': 'Actual:',
        'ip_selector_adb': 'ADB:',
        'ip_selector_tailscale': 'Tailscale:',
        'ip_selector_tailscale_device': 'Tailscale ({0}):',
        'ip_selector_lan': 'LAN:',
        'fps_label': 'FPS: {0:.1f}',
        'language_app': 'IDIOMA Y CONFIG. APP',
        'telegram_notif_app': 'BOT TELEGRAM NOTIF.',
        'ep32_bt_on': 'EP32 BT: ON',
        'ep32_bt_off': 'EP32 BT: OFF',
        'ep32_controls_title': 'Control EP32',
        'ep32_status_label': 'Estado',
        'ep32_status_unknown': 'sin datos',
        'ep32_status_off': 'apagado',
        'ep32_status_idle': 'puente activo, sin comandos',
        'ep32_status_bridge_unreachable': 'cliente Android no responde',
        'ep32_status_invalid_url': 'URL del movil invalida',
        'ep32_status_scanning': 'escaneando',
        'ep32_status_connecting': 'conectando',
        'ep32_status_connected': 'conectado',
        'ep32_status_not_connected': 'no conectado',
        'ep32_status_error': 'error',
        'ep32_status_invalid_payload': 'payload invalido',
        'ep32_status_method_not_allowed': 'metodo no permitido',
        'ep32_status_invalid_command': 'comando invalido',
        'ep32_status_invalid_sequence': 'secuencia invalida',
        'ep32_status_invalid_action': 'accion invalida',
        'ep32_btn_up': 'Arriba',
        'ep32_btn_down': 'Abajo',
        'ep32_btn_left': 'Izqda',
        'ep32_btn_right': 'Dcha',
        'ep32_btn_a': 'A',
        'ep32_btn_b': 'B',
        'ep32_btn_menu': 'Menu',
        'ep32_btn_auto': 'Auto',
        'ep32_btn_fix_a': 'Fijar A',
        'ep32_btn_fix_b': 'Fijar B',
        'ep32_btn_auto_a': 'Auto A',
        'ep32_btn_auto_b': 'Auto B',
        'ep32_autotrack_on': 'AUTO-TRACK: ON',
        'ep32_autotrack_off': 'AUTO-TRACK: OFF',
        'no_streaming': 'Streaming no detectado',
        'no_streaming_yolo': 'No se puede iniciar YOLO porque no hay streaming de video disponible.',
        'activate_audio_first': 'Activa primero la transmisión de sonido pulsando el icono de volumen.',
        'activate_audio_title': 'Activar transmisión de audio',
        'no_signal': 'SIN CONEXION',
        'reconnecting': 'Intentando reconectar...',
        'audio_drone_detected': 'AUDIO DRON DETECTED: {0}%',
        'no_audio_dron': 'NO AUDIO DRON: {0}%',
        'rf_drone_detected': 'DRON DETECTADO POR RF: {0:.3f} GHz ({1}%)',
        'rf_drone_detected_no_freq': 'DRON DETECTADO POR RF ({0}%)',
        'tinysa_connected': 'TinySA conectado',
        'tinysa_connected_android': 'TinySA conectado a Android',
        'adb_connected': 'ADB conectado',
        'tinysa_not_configured': 'TinySA no configurado',
        'configure_tinysa_first': 'Configura TinySA primero usando el botón de engranaje.',
        'tinysa_not_detected': 'TinySA no detectado localmente ni en el servidor Android.\nConéctalo vía USB al PC o al Android e intenta de nuevo.',
        'change_camera_ip': 'Cambiar Cámara IP',
        'enter_new_ip': 'Introduce la nueva IP y puerto:\n(actual: {0})',
        'yolo_options_title': 'Opciones de YOLO',
        'available_models': 'Modelos disponibles',
        'model': 'Modelo {0}',
        'description': 'Descripción:',
        'browse': 'Examinar',
        'select_yolo_model': 'Seleccionar modelo YOLO',
        'yolo_models': 'Modelos YOLO (*.pt)',
        'all_files': 'Todos los archivos',
        'page': 'Página {0}/{1}',
        'load_model': 'Cargar modelo',
        'load_and_save_default': 'Cargar y guardar por defecto',
        'load_default_config': 'Cargar configuración por defecto',
        'cancel': 'Cancelar',
        'model_updated': 'Modelo actualizado correctamente.',
        'error': 'Error',
        'model_empty': 'El modelo del slot {0} está vacío.',
        'file_not_found': 'No se encontró el archivo:\n{0}',
        'tinysa_mode_selection': 'TinySA - Selección de modo',
        'select_mode': 'Selecciona un modo:',
        'fpv_normal': 'FPV - Deteccion por patron (2.442 GHz)',
        'fpv_5g_detection_mode': 'FPV - Deteccion por anomalias espectrales (5 Ghz)',
        'custom_range': 'Rango personalizado por anomalia espectral',
        'start_mhz': 'Inicio (MHz):',
        'stop_mhz': 'Fin (MHz):',
        'advanced_range': 'Intervalos personalizados por anomalia espectral',
        'ok': 'OK',
        'advanced_interval_title': 'Rango personalizado - Intervalo avanzado',
        'up_to_8_intervals': 'Hasta 8 intervalos (MHz)',
        'interval': 'Intervalo {0}:',
        'start': 'Inicio',
        'stop': 'Fin',
        'sweeps': '# barridos',
        'complete_start_end': 'Completa inicio y fin para el intervalo {0}.',
        'invalid_values': 'Valores inválidos en intervalo {0}.',
        'end_must_be_greater': 'El fin debe ser mayor que el inicio en el intervalo {0}.',
        'sweeps_must_be_positive': 'El número de barridos debe ser mayor a cero (intervalo {0}).',
        'enter_valid_interval': 'Introduce al menos un intervalo válido.',
        'enter_numeric_values': 'Introduce valores numéricos válidos para inicio y fin.',
        'end_greater_than_start': 'El fin debe ser mayor que el inicio.',
        'language_selection_title': 'Seleccionar Idioma',
        'select_language': 'Selecciona un idioma:',
        'could_not_save_language': 'No se pudo guardar el idioma.',
        'tailscale_on': 'TAILSCALE: ON',
        'tailscale_off': 'TAILSCALE: OFF',
        'tailscale_config_title': 'Configuración Tailscale',
        # 'tailscale_username' y 'tailscale_password' eliminados - no se usan (Tailscale maneja auth)
        'tailscale_not_configured': 'Tailscale no configurado',
        'configure_tailscale_first': 'Configura Tailscale primero usando el botón de engranaje.',
        'tailscale_connecting': 'Conectando a Tailscale...',
        'tailscale_disconnecting': 'Desconectando de Tailscale...',
        'tailscale_connected': 'Tailscale conectado correctamente.',
        'tailscale_disconnected': 'Tailscale desconectado correctamente.',
        'tailscale_error': 'Error al conectar/desconectar Tailscale.',
        'tailscale_state_store_error': 'Tailscale no pudo inicializar su almacenamiento seguro (TPM). Abre Tailscale como administrador y sigue la guia oficial: https://tailscale.com/s/state-store-init-error . Despues reinicia el servicio Tailscale o reinstala Tailscale.',
        'tailscale_not_installed': 'Tailscale no está instalado. Instálalo desde https://tailscale.com/download',
        'install_tailscale': 'Instalar servicio',
        'installing_tailscale': 'Instalando Tailscale...',
        'tailscale_install_wait_message': 'Instalando Tailscale, espere unos segundos.',
        'tailscale_service_installed': 'Servicio de Tailscale instalado.',
        'tailscale_install_success': 'Tailscale instalado correctamente. Reinicia la aplicación.',
        'tailscale_install_error': 'Error al instalar Tailscale.',
        'tailscale_installer_not_found': 'Instalador de Tailscale no encontrado en la ruta del proyecto.',
        'tailscale_oauth_info': 'Tailscale usa autenticación OAuth (Google, Microsoft, etc.).\nAl activar Tailscale, se abrirá tu navegador para autenticarte.',
        'tailscale_installed_info': 'Tailscale está instalado. Usa el botón TAILSCALE: ON/OFF para conectarte.',
        'tailscale_logged_in_as': 'Logueado como:',
        'tailscale_ip_device': 'IP Tailscale de este dispositivo:',
        'tailscale_other_devices': 'Otros dispositivos conectados:',
        'tailscale_create_account': 'Crear nueva cuenta Tailscale',
        'tailscale_sudo_needed': 'Tailscale requiere permisos de administrador.\n\nEjecuta una vez en la terminal:\nsudo tailscale set --operator=$USER\n\nLuego podrás usar Tailscale sin sudo.',
        'nvidia_cuda_info': 'Se recomienda si se dispone de tarjeta gráfica NVIDIA, tener los drivers actualizados. Además, en Linux se requiere NVIDIA Cuda Toolkit para correcto funcionamiento de las librerías (sudo apt install nvidia-cuda-toolkit).',
        'audio_sensitivity_label': 'Sensibilidad raw de detección de audio por Tensorflow (avanzado):',
        'audio_sensitivity_percent': '%',
        'audio_sensitivity_save_error': 'No se pudo guardar la sensibilidad de audio.',
        'audio_sensitivity_range_error': 'El valor de sensibilidad debe estar entre 1% y 100%.',
        'audio_sensitivity_number_error': 'El valor de sensibilidad debe ser un número.',
        'rf_flat_loading_message': 'Cargando deteccion plana - Asegurate de la no presencia de drones',
        'rf_flat_loading_progress': 'Calibrando baseline: {0}/{1}',
        'rf_flat_loading_progress_interval': 'Intervalo {0}/{1}: {2}/{3}',
        'telegram_config_load_error': 'No se pudo cargar la configuración de Telegram.',
        'telegram_config_save_error': 'No se pudo guardar la configuración de Telegram.',
        'telegram_enabled_start': '[TELEGRAM] Notificaciones activas.',
        'telegram_disabled_start': '[TELEGRAM] Notificaciones desactivadas (edita telegram_config.json para activarlas).',
        'telegram_yolo_message': 'YOLO detectó un dron a las {0}.',
        'telegram_yolo_message_conf': 'YOLO detectó un dron a las {0} (confianza {1}%).',
        'telegram_rf_message': 'RF detectó señal compatible con dron a las {0} (confianza {1}%).',
        'telegram_rf_message_freq': 'RF detectó señal compatible con dron a las {0}: {1:.3f} MHz ({2}%).',
        'telegram_audio_message': 'Tensorflow detectó patrón de dron a las {0} (confianza {1}%).',
        'telegram_audio_size_suffix': 'Firma sonora: {0} ({1}%)',
        'drone_size_small': 'dron pequeño',
        'drone_size_medium': 'dron mediano',
        'drone_size_large': 'dron grande',
        'drone_size_inconclusive': 'tipo no concluyente',
        'telegram_window_title': 'Configuración de bot Telegram',
        'telegram_save_success': 'Configuración de Telegram guardada correctamente.',
        'telegram_section_title': 'Bot de Telegram',
        'telegram_setup_help': '1) Pulsa BotFather y crea el bot.\n2) Copia el token aquí.\n3) Envía /start a tu bot y pulsa "Detectar chat_id".',
        'telegram_enable_notifications': 'Activar notificaciones por Telegram',
        'telegram_bot_token': 'Bot token:',
        'telegram_chat_id': 'Chat ID:',
        'telegram_detect_chat_id': 'Detectar chat_id',
        'telegram_open_botfather': 'Abrir BotFather',
        'telegram_media_options': 'Adjuntos en alertas',
        'telegram_send_yolo_photo': 'Enviar foto en detecciones YOLO',
        'telegram_send_rf_image': 'Enviar imagen espectrograma RF',
        'telegram_send_audio_clip': 'Enviar clip de audio en detección sonora',
        'telegram_cooldowns_label': 'Cooldown por tipo (segundos)',
        'telegram_cooldown_yolo': 'YOLO:',
        'telegram_cooldown_rf': 'RF:',
        'telegram_cooldown_audio': 'Audio:',
        'telegram_cooldown_number_error': 'Los cooldowns deben ser números enteros.',
        'telegram_cooldown_range_error': 'Los cooldowns no pueden ser negativos.',
        'telegram_required_fields_error': 'Para activar Telegram, completa token y chat_id.',
        'telegram_chat_id_missing_token': 'Introduce primero el token del bot.',
        'telegram_chat_id_detect_error': 'No se pudo detectar el chat_id automáticamente.',
        'telegram_chat_id_not_found': 'No hay mensajes recientes. Abre tu bot y envía /start, luego reintenta.',
        'telegram_chat_id_detected': 'Chat ID detectado: {0}',
    },
    'en': {  # English
        'yolo_on': 'YOLO: {0} det.',
        'yolo_off': 'YOLO: OFF',
        'det_power_label': 'DET.POWER',
        'det_power_short_low': 'L',
        'det_power_short_med': 'M',
        'det_power_short_high': 'H',
        'det_power_short_vhigh': 'VH',
        'tinysa_on': 'TinySA: ON',
        'tinysa_off': 'TinySA: OFF',
        'det_audio_on': 'AUDIO DET: ON',
        'det_audio_off': 'AUDIO DET: OFF',
        'audio_source_phone_mic': 'AUDIO: PHONE MIC',
        'audio_source_esp32_array': 'AUDIO: ESP32 ARRAY',
        'ip_label': 'IP: {0}',
        'ip_selector_title': 'IP Selector',
        'ip_selector_prompt': 'Select an available IP:',
        'ip_selector_no_candidates': 'No candidate IPs found in Tailscale or local network.',
        'ip_selector_no_selection': 'Select an IP from the list.',
        'ip_selector_current': 'Current:',
        'ip_selector_adb': 'ADB:',
        'ip_selector_tailscale': 'Tailscale:',
        'ip_selector_tailscale_device': 'Tailscale ({0}):',
        'ip_selector_lan': 'LAN:',
        'fps_label': 'FPS: {0:.1f}',
        'language_app': 'LANGUAGE & APP CONFIG.',
        'telegram_notif_app': 'TELEGRAM BOT NOTIF.',
        'ep32_bt_on': 'EP32 BT: ON',
        'ep32_bt_off': 'EP32 BT: OFF',
        'ep32_controls_title': 'EP32 Control',
        'ep32_status_label': 'Status',
        'ep32_status_unknown': 'unknown',
        'ep32_status_off': 'off',
        'ep32_status_idle': 'bridge ready, no commands sent',
        'ep32_status_bridge_unreachable': 'Android client not responding',
        'ep32_status_invalid_url': 'invalid phone URL',
        'ep32_status_scanning': 'scanning',
        'ep32_status_connecting': 'connecting',
        'ep32_status_connected': 'connected',
        'ep32_status_not_connected': 'not connected',
        'ep32_status_error': 'error',
        'ep32_status_invalid_payload': 'invalid payload',
        'ep32_status_method_not_allowed': 'method not allowed',
        'ep32_status_invalid_command': 'invalid command',
        'ep32_status_invalid_sequence': 'invalid sequence',
        'ep32_status_invalid_action': 'invalid action',
        'ep32_btn_up': 'Up',
        'ep32_btn_down': 'Down',
        'ep32_btn_left': 'Left',
        'ep32_btn_right': 'Right',
        'ep32_btn_a': 'A',
        'ep32_btn_b': 'B',
        'ep32_btn_menu': 'Menu',
        'ep32_btn_auto': 'Auto',
        'ep32_btn_fix_a': 'Fix A',
        'ep32_btn_fix_b': 'Fix B',
        'ep32_btn_auto_a': 'Auto A',
        'ep32_btn_auto_b': 'Auto B',
        'ep32_autotrack_on': 'AUTO-TRACK: ON',
        'ep32_autotrack_off': 'AUTO-TRACK: OFF',
        'no_streaming': 'Streaming not detected',
        'no_streaming_yolo': 'Cannot start YOLO because there is no video streaming available.',
        'activate_audio_first': 'Activate audio transmission first by clicking the volume icon.',
        'activate_audio_title': 'Activate audio transmission',
        'no_signal': 'NO SIGNAL',
        'reconnecting': 'Trying to reconnect...',
        'audio_drone_detected': 'AUDIO DRONE DETECTED: {0}%',
        'no_audio_dron': 'NO AUDIO DRONE: {0}%',
        'rf_drone_detected': 'DRONE DETECTED BY RF: {0:.3f} GHz ({1}%)',
        'rf_drone_detected_no_freq': 'DRONE DETECTED BY RF ({0}%)',
        'tinysa_connected': 'TinySA connected',
        'tinysa_connected_android': 'TinySA connected to Android',
        'adb_connected': 'ADB connected',
        'tinysa_not_configured': 'TinySA not configured',
        'configure_tinysa_first': 'Configure TinySA first using the gear button.',
        'tinysa_not_detected': 'TinySA not detected locally or on Android server.\nConnect it via USB to PC or Android and try again.',
        'change_camera_ip': 'Change Camera IP',
        'enter_new_ip': 'Enter new IP and port:\n(current: {0})',
        'yolo_options_title': 'YOLO Options',
        'available_models': 'Available Models',
        'model': 'Model {0}',
        'description': 'Description:',
        'browse': 'Browse',
        'select_yolo_model': 'Select YOLO Model',
        'yolo_models': 'YOLO Models (*.pt)',
        'all_files': 'All Files',
        'page': 'Page {0}/{1}',
        'load_model': 'Load Model',
        'load_and_save_default': 'Load and Save as Default',
        'load_default_config': 'Load Default Configuration',
        'cancel': 'Cancel',
        'model_updated': 'Model updated successfully.',
        'error': 'Error',
        'model_empty': 'Model in slot {0} is empty.',
        'file_not_found': 'File not found:\n{0}',
        'tinysa_mode_selection': 'TinySA - Mode Selection',
        'select_mode': 'Select a mode:',
        'fpv_normal': 'FPV - Pattern-based detection (2.442 GHz)',
        'fpv_5g_detection_mode': 'FPV - Spectral anomaly detection (5 GHz)',
        'custom_range': 'Custom range by spectral anomaly',
        'start_mhz': 'Start (MHz):',
        'stop_mhz': 'Stop (MHz):',
        'advanced_range': 'Custom intervals by spectral anomaly',
        'ok': 'OK',
        'advanced_interval_title': 'Custom Range - Advanced Interval',
        'up_to_8_intervals': 'Up to 8 intervals (MHz)',
        'interval': 'Interval {0}:',
        'start': 'Start',
        'stop': 'Stop',
        'sweeps': '# sweeps',
        'complete_start_end': 'Complete start and end for interval {0}.',
        'invalid_values': 'Invalid values in interval {0}.',
        'end_must_be_greater': 'End must be greater than start in interval {0}.',
        'sweeps_must_be_positive': 'Number of sweeps must be greater than zero (interval {0}).',
        'enter_valid_interval': 'Enter at least one valid interval.',
        'enter_numeric_values': 'Enter valid numeric values for start and end.',
        'end_greater_than_start': 'End must be greater than start.',
        'language_selection_title': 'Select Language',
        'select_language': 'Select a language:',
        'could_not_save_language': 'Could not save language.',
        'tailscale_on': 'TAILSCALE: ON',
        'tailscale_off': 'TAILSCALE: OFF',
        'tailscale_config_title': 'Tailscale Configuration',
        # 'tailscale_username' y 'tailscale_password' eliminados - no se usan
        'tailscale_not_configured': 'Tailscale not configured',
        'configure_tailscale_first': 'Configure Tailscale first using the gear button.',
        'tailscale_connecting': 'Connecting to Tailscale...',
        'tailscale_disconnecting': 'Disconnecting from Tailscale...',
        'tailscale_connected': 'Tailscale connected successfully.',
        'tailscale_disconnected': 'Tailscale disconnected successfully.',
        'tailscale_error': 'Error connecting/disconnecting Tailscale.',
        'tailscale_state_store_error': 'Tailscale could not initialize its secure state store (TPM). Open Tailscale as administrator and follow the official guide: https://tailscale.com/s/state-store-init-error . Then restart the Tailscale service or reinstall Tailscale.',
        'tailscale_not_installed': 'Tailscale is not installed. Install it from https://tailscale.com/download',
        'install_tailscale': 'Install Service',
        'installing_tailscale': 'Installing Tailscale...',
        'tailscale_install_wait_message': 'Installing Tailscale, please wait a few seconds.',
        'tailscale_service_installed': 'Tailscale service installed.',
        'tailscale_install_success': 'Tailscale installed successfully. Restart the application.',
        'tailscale_install_error': 'Error installing Tailscale.',
        'tailscale_installer_not_found': 'Tailscale installer not found in project path.',
        'tailscale_oauth_info': 'Tailscale uses OAuth authentication (Google, Microsoft, etc.).\nWhen you activate Tailscale, your browser will open for authentication.',
        'tailscale_installed_info': 'Tailscale is installed. Use the TAILSCALE: ON/OFF button to connect.',
        'tailscale_logged_in_as': 'Logged in as:',
        'tailscale_ip_device': 'Tailscale IP of this device:',
        'tailscale_other_devices': 'Other connected devices:',
        'tailscale_create_account': 'Create new Tailscale account',
        'tailscale_sudo_needed': 'Tailscale requires administrator permissions.\n\nRun once in terminal:\nsudo tailscale set --operator=$USER\n\nThen you can use Tailscale without sudo.',
        'nvidia_cuda_info': 'It is recommended that if you have an NVIDIA graphics card, keep the drivers updated. Also, on Linux, NVIDIA CUDA Toolkit is required for proper functioning of the libraries (sudo apt install nvidia-cuda-toolkit).',
        'audio_sensitivity_label': 'Raw audio detection sensitivity by Tensorflow (advanced):',
        'audio_sensitivity_percent': '%',
        'audio_sensitivity_save_error': 'Could not save audio sensitivity.',
        'audio_sensitivity_range_error': 'Sensitivity value must be between 1% and 100%.',
        'audio_sensitivity_number_error': 'Sensitivity value must be a number.',
        'rf_flat_loading_message': 'Loading flat detection - Ensure no drones are present',
        'rf_flat_loading_progress': 'Calibrating baseline: {0}/{1}',
        'rf_flat_loading_progress_interval': 'Interval {0}/{1}: {2}/{3}',
        'telegram_config_load_error': 'Could not load Telegram configuration.',
        'telegram_config_save_error': 'Could not save Telegram configuration.',
        'telegram_enabled_start': '[TELEGRAM] Notifications enabled.',
        'telegram_disabled_start': '[TELEGRAM] Notifications disabled (edit telegram_config.json to enable).',
        'telegram_yolo_message': 'YOLO detected a drone at {0}.',
        'telegram_yolo_message_conf': 'YOLO detected a drone at {0} (confidence {1}%).',
        'telegram_rf_message': 'RF detected drone-like signal at {0} (confidence {1}%).',
        'telegram_rf_message_freq': 'RF detected drone-like signal at {0}: {1:.3f} MHz ({2}%).',
        'telegram_audio_message': 'Tensorflow detected drone-like pattern at {0} (confidence {1}%).',
        'telegram_audio_size_suffix': 'Sound signature: {0} ({1}%)',
        'drone_size_small': 'small drone',
        'drone_size_medium': 'medium drone',
        'drone_size_large': 'large drone',
        'drone_size_inconclusive': 'inconclusive type',
        'telegram_window_title': 'Telegram bot settings',
        'telegram_save_success': 'Telegram settings saved successfully.',
        'telegram_section_title': 'Telegram Bot',
        'telegram_setup_help': '1) Click BotFather and create the bot.\n2) Paste token here.\n3) Send /start to your bot and click "Detect chat_id".',
        'telegram_enable_notifications': 'Enable Telegram notifications',
        'telegram_bot_token': 'Bot token:',
        'telegram_chat_id': 'Chat ID:',
        'telegram_detect_chat_id': 'Detect chat_id',
        'telegram_open_botfather': 'Open BotFather',
        'telegram_media_options': 'Alert attachments',
        'telegram_send_yolo_photo': 'Send photo for YOLO detections',
        'telegram_send_rf_image': 'Send RF spectrogram image',
        'telegram_send_audio_clip': 'Send audio clip for audio detections',
        'telegram_cooldowns_label': 'Per-type cooldown (seconds)',
        'telegram_cooldown_yolo': 'YOLO:',
        'telegram_cooldown_rf': 'RF:',
        'telegram_cooldown_audio': 'Audio:',
        'telegram_cooldown_number_error': 'Cooldown values must be integer numbers.',
        'telegram_cooldown_range_error': 'Cooldown values cannot be negative.',
        'telegram_required_fields_error': 'To enable Telegram, fill token and chat_id.',
        'telegram_chat_id_missing_token': 'Enter bot token first.',
        'telegram_chat_id_detect_error': 'Could not auto-detect chat_id.',
        'telegram_chat_id_not_found': 'No recent messages found. Open your bot, send /start, and retry.',
        'telegram_chat_id_detected': 'Detected Chat ID: {0}',
    },
    'fr': {  # French
        'yolo_on': 'YOLO: {0} dét.',
        'yolo_off': 'YOLO: OFF',
        'det_power_label': 'DET.POWER',
        'det_power_short_low': 'L',
        'det_power_short_med': 'M',
        'det_power_short_high': 'H',
        'det_power_short_vhigh': 'VH',
        'tinysa_on': 'TinySA: ON',
        'tinysa_off': 'TinySA: OFF',
        'det_audio_on': 'DET AUDIO: ON',
        'det_audio_off': 'DET AUDIO: OFF',
        'audio_source_phone_mic': 'AUDIO: MIC MOBILE',
        'audio_source_esp32_array': 'AUDIO: ARRAY ESP32',
        'ip_label': 'IP: {0}',
        'ip_selector_title': 'Selecteur IP',
        'ip_selector_prompt': 'Selectionnez une IP disponible :',
        'ip_selector_no_candidates': 'Aucune IP candidate trouvee dans Tailscale ou le reseau local.',
        'ip_selector_no_selection': 'Selectionnez une IP dans la liste.',
        'ip_selector_current': 'Actuelle :',
        'ip_selector_adb': 'ADB :',
        'ip_selector_tailscale': 'Tailscale :',
        'ip_selector_tailscale_device': 'Tailscale ({0}) :',
        'ip_selector_lan': 'LAN :',
        'fps_label': 'FPS: {0:.1f}',
        'language_app': 'LANGUE ET CONFIG. APP',
        'telegram_notif_app': 'BOT TELEGRAM NOTIF.',
        'ep32_bt_on': 'EP32 BT: ON',
        'ep32_bt_off': 'EP32 BT: OFF',
        'ep32_controls_title': 'Controle EP32',
        'ep32_status_label': 'Statut',
        'ep32_status_unknown': 'inconnu',
        'ep32_status_off': 'eteint',
        'ep32_status_idle': 'pont pret, aucune commande',
        'ep32_status_bridge_unreachable': 'client Android injoignable',
        'ep32_status_invalid_url': 'URL du telephone invalide',
        'ep32_status_scanning': 'scan',
        'ep32_status_connecting': 'connexion',
        'ep32_status_connected': 'connecte',
        'ep32_status_not_connected': 'non connecte',
        'ep32_status_error': 'erreur',
        'ep32_status_invalid_payload': 'payload invalide',
        'ep32_status_method_not_allowed': 'methode non autorisee',
        'ep32_status_invalid_command': 'commande invalide',
        'ep32_status_invalid_sequence': 'sequence invalide',
        'ep32_status_invalid_action': 'action invalide',
        'ep32_btn_up': 'Haut',
        'ep32_btn_down': 'Bas',
        'ep32_btn_left': 'Gauche',
        'ep32_btn_right': 'Droite',
        'ep32_btn_a': 'A',
        'ep32_btn_b': 'B',
        'ep32_btn_menu': 'Menu',
        'ep32_btn_auto': 'Auto',
        'ep32_btn_fix_a': 'Fixer A',
        'ep32_btn_fix_b': 'Fixer B',
        'ep32_btn_auto_a': 'Auto A',
        'ep32_btn_auto_b': 'Auto B',
        'ep32_autotrack_on': 'AUTO-TRACK: ON',
        'ep32_autotrack_off': 'AUTO-TRACK: OFF',
        'no_streaming': 'Streaming non détecté',
        'no_streaming_yolo': 'Impossible de démarrer YOLO car aucun streaming vidéo n\'est disponible.',
        'activate_audio_first': 'Activez d\'abord la transmission audio en cliquant sur l\'icône de volume.',
        'activate_audio_title': 'Activer la transmission audio',
        'no_signal': 'PAS DE SIGNAL',
        'reconnecting': 'Tentative de reconnexion...',
        'audio_drone_detected': 'DRONE AUDIO DÉTECTÉ: {0}%',
        'no_audio_dron': 'PAS DE DRONE AUDIO: {0}%',
        'rf_drone_detected': 'DRONE DÉTECTÉ PAR RF: {0:.3f} GHz ({1}%)',
        'rf_drone_detected_no_freq': 'DRONE DÉTECTÉ PAR RF ({0}%)',
        'tinysa_connected': 'TinySA connecté',
        'tinysa_connected_android': 'TinySA connecté à Android',
        'adb_connected': 'ADB connecté',
        'tinysa_not_configured': 'TinySA non configuré',
        'configure_tinysa_first': 'Configurez d\'abord TinySA en utilisant le bouton d\'engrenage.',
        'tinysa_not_detected': 'TinySA non détecté localement ni sur le serveur Android.\nConnectez-le via USB au PC ou à Android et réessayez.',
        'change_camera_ip': 'Changer IP Caméra',
        'enter_new_ip': 'Entrez le nouvel IP et port:\n(actuel: {0})',
        'yolo_options_title': 'Options YOLO',
        'available_models': 'Modèles disponibles',
        'model': 'Modèle {0}',
        'description': 'Description:',
        'browse': 'Parcourir',
        'select_yolo_model': 'Sélectionner modèle YOLO',
        'yolo_models': 'Modèles YOLO (*.pt)',
        'all_files': 'Tous les fichiers',
        'page': 'Page {0}/{1}',
        'load_model': 'Charger modèle',
        'load_and_save_default': 'Charger et enregistrer par défaut',
        'load_default_config': 'Charger configuration par défaut',
        'cancel': 'Annuler',
        'model_updated': 'Modèle mis à jour avec succès.',
        'error': 'Erreur',
        'model_empty': 'Le modèle dans l\'emplacement {0} est vide.',
        'file_not_found': 'Fichier non trouvé:\n{0}',
        'tinysa_mode_selection': 'TinySA - Sélection de mode',
        'select_mode': 'Sélectionnez un mode:',
        'fpv_normal': 'FPV - Detection par motif (2.442 GHz)',
        'fpv_5g_detection_mode': 'FPV - Detection par anomalies spectrales (5 GHz)',
        'custom_range': 'Plage personnalisee par anomalie spectrale',
        'start_mhz': 'Début (MHz):',
        'stop_mhz': 'Fin (MHz):',
        'advanced_range': 'Intervalles personnalises par anomalie spectrale',
        'ok': 'OK',
        'advanced_interval_title': 'Plage personnalisée - Intervalle avancé',
        'up_to_8_intervals': 'Jusqu\'à 8 intervalles (MHz)',
        'interval': 'Intervalle {0}:',
        'start': 'Début',
        'stop': 'Fin',
        'sweeps': '# balayages',
        'complete_start_end': 'Complétez début et fin pour l\'intervalle {0}.',
        'invalid_values': 'Valeurs invalides dans l\'intervalle {0}.',
        'end_must_be_greater': 'La fin doit être supérieure au début dans l\'intervalle {0}.',
        'sweeps_must_be_positive': 'Le nombre de balayages doit être supérieur à zéro (intervalle {0}).',
        'enter_valid_interval': 'Entrez au moins un intervalle valide.',
        'enter_numeric_values': 'Entrez des valeurs numériques valides pour début et fin.',
        'end_greater_than_start': 'La fin doit être supérieure au début.',
        'language_selection_title': 'Sélectionner Langue',
        'select_language': 'Sélectionnez une langue:',
        'could_not_save_language': 'Impossible d\'enregistrer la langue.',
        'tailscale_on': 'TAILSCALE: ON',
        'tailscale_off': 'TAILSCALE: OFF',
        'tailscale_config_title': 'Configuration Tailscale',
        # 'tailscale_username' y 'tailscale_password' eliminados - no se usan
        'tailscale_not_configured': 'Tailscale non configuré',
        'configure_tailscale_first': 'Configurez Tailscale d\'abord en utilisant le bouton d\'engrenage.',
        'tailscale_connecting': 'Connexion à Tailscale...',
        'tailscale_disconnecting': 'Déconnexion de Tailscale...',
        'tailscale_connected': 'Tailscale connecté avec succès.',
        'tailscale_disconnected': 'Tailscale déconnecté avec succès.',
        'tailscale_error': 'Erreur lors de la connexion/déconnexion de Tailscale.',
        'tailscale_state_store_error': 'Tailscale n a pas pu initialiser son stockage securise (TPM). Ouvrez Tailscale en administrateur et suivez le guide officiel : https://tailscale.com/s/state-store-init-error . Ensuite, redemarrez le service Tailscale ou reinstallez Tailscale.',
        'tailscale_not_installed': 'Tailscale n\'est pas installé. Installez-le depuis https://tailscale.com/download',
        'install_tailscale': 'Installer le service',
        'installing_tailscale': 'Installation de Tailscale...',
        'tailscale_install_wait_message': 'Installation de Tailscale, veuillez patienter quelques secondes.',
        'tailscale_service_installed': 'Service Tailscale installe.',
        'tailscale_install_success': 'Tailscale installé avec succès. Redémarrez l\'application.',
        'tailscale_install_error': 'Erreur lors de l\'installation de Tailscale.',
        'tailscale_installer_not_found': 'Installateur Tailscale introuvable dans le chemin du projet.',
        'tailscale_oauth_info': 'Tailscale utilise l\'authentification OAuth (Google, Microsoft, etc.).\nLorsque vous activez Tailscale, votre navigateur s\'ouvrira pour l\'authentification.',
        'tailscale_installed_info': 'Tailscale est installé. Utilisez le bouton TAILSCALE: ON/OFF pour vous connecter.',
        'tailscale_logged_in_as': 'Connecté en tant que:',
        'tailscale_ip_device': 'IP Tailscale de cet appareil:',
        'tailscale_other_devices': 'Autres appareils connectés:',
        'tailscale_create_account': 'Créer un nouveau compte Tailscale',
        'tailscale_sudo_needed': 'Tailscale nécessite des permissions d\'administrateur.\n\nExécutez une fois dans le terminal:\nsudo tailscale set --operator=$USER\n\nEnsuite, vous pourrez utiliser Tailscale sans sudo.',
        'nvidia_cuda_info': 'Il est recommandé, si vous disposez d\'une carte graphique NVIDIA, d\'avoir les pilotes à jour. De plus, sur Linux, NVIDIA CUDA Toolkit est requis pour le bon fonctionnement des bibliothèques (sudo apt install nvidia-cuda-toolkit).',
        'audio_sensitivity_label': 'Sensibilité brute de détection audio par Tensorflow (avancé):',
        'audio_sensitivity_percent': '%',
        'audio_sensitivity_save_error': 'Impossible d\'enregistrer la sensibilité audio.',
        'audio_sensitivity_range_error': 'La valeur de sensibilité doit être comprise entre 1% et 100%.',
        'audio_sensitivity_number_error': 'La valeur de sensibilité doit être un nombre.',
        'rf_flat_loading_message': 'Chargement détection plate - Assurez l\'absence de drones',
        'rf_flat_loading_progress': 'Calibration baseline : {0}/{1}',
        'rf_flat_loading_progress_interval': 'Intervalle {0}/{1} : {2}/{3}',
        'telegram_config_load_error': 'Impossible de charger la configuration Telegram.',
        'telegram_config_save_error': 'Impossible d\'enregistrer la configuration Telegram.',
        'telegram_enabled_start': '[TELEGRAM] Notifications activées.',
        'telegram_disabled_start': '[TELEGRAM] Notifications désactivées (modifiez telegram_config.json pour activer).',
        'telegram_yolo_message': 'YOLO a détecté un drone à {0}.',
        'telegram_yolo_message_conf': 'YOLO a détecté un drone à {0} (confiance {1}%).',
        'telegram_rf_message': 'RF a détecté un signal de drone à {0} (confiance {1}%).',
        'telegram_rf_message_freq': 'RF a détecté un signal de drone à {0} : {1:.3f} MHz ({2}%).',
        'telegram_audio_message': 'Tensorflow a détecté un motif de drone à {0} (confiance {1}%).',
        'telegram_audio_size_suffix': 'Signature sonore: {0} ({1}%)',
        'drone_size_small': 'petit drone',
        'drone_size_medium': 'drone moyen',
        'drone_size_large': 'gros drone',
        'drone_size_inconclusive': 'type non concluant',
        'telegram_window_title': 'Configuration du bot Telegram',
        'telegram_save_success': 'Configuration Telegram enregistrée avec succès.',
        'telegram_section_title': 'Bot Telegram',
        'telegram_setup_help': '1) Ouvrez BotFather et créez le bot.\n2) Collez le token ici.\n3) Envoyez /start à votre bot puis cliquez sur "Détecter chat_id".',
        'telegram_enable_notifications': 'Activer les notifications Telegram',
        'telegram_bot_token': 'Token du bot :',
        'telegram_chat_id': 'Chat ID :',
        'telegram_detect_chat_id': 'Détecter chat_id',
        'telegram_open_botfather': 'Ouvrir BotFather',
        'telegram_media_options': 'Pièces jointes des alertes',
        'telegram_send_yolo_photo': 'Envoyer photo pour détections YOLO',
        'telegram_send_rf_image': 'Envoyer image spectrogramme RF',
        'telegram_send_audio_clip': 'Envoyer clip audio pour détection sonore',
        'telegram_cooldowns_label': 'Cooldown par type (secondes)',
        'telegram_cooldown_yolo': 'YOLO :',
        'telegram_cooldown_rf': 'RF :',
        'telegram_cooldown_audio': 'Audio :',
        'telegram_cooldown_number_error': 'Les cooldowns doivent être des nombres entiers.',
        'telegram_cooldown_range_error': 'Les cooldowns ne peuvent pas être négatifs.',
        'telegram_required_fields_error': 'Pour activer Telegram, renseignez token et chat_id.',
        'telegram_chat_id_missing_token': 'Entrez d\'abord le token du bot.',
        'telegram_chat_id_detect_error': 'Impossible de détecter automatiquement chat_id.',
        'telegram_chat_id_not_found': 'Aucun message récent. Ouvrez votre bot, envoyez /start, puis réessayez.',
        'telegram_chat_id_detected': 'Chat ID détecté : {0}',
    },
    'it': {  # Italian
        'yolo_on': 'YOLO: {0} rilev.',
        'yolo_off': 'YOLO: OFF',
        'det_power_label': 'DET.POWER',
        'det_power_short_low': 'L',
        'det_power_short_med': 'M',
        'det_power_short_high': 'H',
        'det_power_short_vhigh': 'VH',
        'tinysa_on': 'TinySA: ON',
        'tinysa_off': 'TinySA: OFF',
        'det_audio_on': 'RIL AUDIO: ON',
        'det_audio_off': 'RIL AUDIO: OFF',
        'audio_source_phone_mic': 'AUDIO: MIC TELEFONO',
        'audio_source_esp32_array': 'AUDIO: ARRAY ESP32',
        'ip_label': 'IP: {0}',
        'ip_selector_title': 'Selettore IP',
        'ip_selector_prompt': 'Seleziona un IP disponibile:',
        'ip_selector_no_candidates': 'Nessun IP candidato trovato in Tailscale o rete locale.',
        'ip_selector_no_selection': 'Seleziona un IP dalla lista.',
        'ip_selector_current': 'Attuale:',
        'ip_selector_adb': 'ADB:',
        'ip_selector_tailscale': 'Tailscale:',
        'ip_selector_tailscale_device': 'Tailscale ({0}):',
        'ip_selector_lan': 'LAN:',
        'fps_label': 'FPS: {0:.1f}',
        'language_app': 'LINGUA E CONFIG. APP',
        'telegram_notif_app': 'BOT TELEGRAM NOTIF.',
        'ep32_bt_on': 'EP32 BT: ON',
        'ep32_bt_off': 'EP32 BT: OFF',
        'ep32_controls_title': 'Controllo EP32',
        'ep32_status_label': 'Stato',
        'ep32_status_unknown': 'sconosciuto',
        'ep32_status_off': 'spento',
        'ep32_status_idle': 'ponte pronto, nessun comando',
        'ep32_status_bridge_unreachable': 'client Android non risponde',
        'ep32_status_invalid_url': 'URL del telefono non valido',
        'ep32_status_scanning': 'scansione',
        'ep32_status_connecting': 'connessione',
        'ep32_status_connected': 'connesso',
        'ep32_status_not_connected': 'non connesso',
        'ep32_status_error': 'errore',
        'ep32_status_invalid_payload': 'payload non valido',
        'ep32_status_method_not_allowed': 'metodo non consentito',
        'ep32_status_invalid_command': 'comando non valido',
        'ep32_status_invalid_sequence': 'sequenza non valida',
        'ep32_status_invalid_action': 'azione non valida',
        'ep32_btn_up': 'Su',
        'ep32_btn_down': 'Giu',
        'ep32_btn_left': 'Sin',
        'ep32_btn_right': 'Des',
        'ep32_btn_a': 'A',
        'ep32_btn_b': 'B',
        'ep32_btn_menu': 'Menu',
        'ep32_btn_auto': 'Auto',
        'ep32_btn_fix_a': 'Fissa A',
        'ep32_btn_fix_b': 'Fissa B',
        'ep32_btn_auto_a': 'Auto A',
        'ep32_btn_auto_b': 'Auto B',
        'ep32_autotrack_on': 'AUTO-TRACK: ON',
        'ep32_autotrack_off': 'AUTO-TRACK: OFF',
        'no_streaming': 'Streaming non rilevato',
        'no_streaming_yolo': 'Impossibile avviare YOLO perché non è disponibile alcuno streaming video.',
        'activate_audio_first': 'Attiva prima la trasmissione audio cliccando sull\'icona del volume.',
        'activate_audio_title': 'Attiva trasmissione audio',
        'no_signal': 'NESSUN SEGNALE',
        'reconnecting': 'Tentativo di riconnessione...',
        'audio_drone_detected': 'DRONE AUDIO RILEVATO: {0}%',
        'no_audio_dron': 'NESSUN DRONE AUDIO: {0}%',
        'rf_drone_detected': 'DRONE RILEVATO DA RF: {0:.3f} GHz ({1}%)',
        'rf_drone_detected_no_freq': 'DRONE RILEVATO DA RF ({0}%)',
        'tinysa_connected': 'TinySA connesso',
        'tinysa_connected_android': 'TinySA connesso ad Android',
        'adb_connected': 'ADB connesso',
        'tinysa_not_configured': 'TinySA non configurato',
        'configure_tinysa_first': 'Configura prima TinySA usando il pulsante ingranaggio.',
        'tinysa_not_detected': 'TinySA non rilevato localmente né sul server Android.\nCollegalo via USB al PC o ad Android e riprova.',
        'change_camera_ip': 'Cambia IP Camera',
        'enter_new_ip': 'Inserisci nuovo IP e porta:\n(attuale: {0})',
        'yolo_options_title': 'Opzioni YOLO',
        'available_models': 'Modelli disponibili',
        'model': 'Modello {0}',
        'description': 'Descrizione:',
        'browse': 'Sfoglia',
        'select_yolo_model': 'Seleziona modello YOLO',
        'yolo_models': 'Modelli YOLO (*.pt)',
        'all_files': 'Tutti i file',
        'page': 'Pagina {0}/{1}',
        'load_model': 'Carica modello',
        'load_and_save_default': 'Carica e salva come predefinito',
        'load_default_config': 'Carica configurazione predefinita',
        'cancel': 'Annulla',
        'model_updated': 'Modello aggiornato correttamente.',
        'error': 'Errore',
        'model_empty': 'Il modello nello slot {0} è vuoto.',
        'file_not_found': 'File non trovato:\n{0}',
        'tinysa_mode_selection': 'TinySA - Selezione modalità',
        'select_mode': 'Seleziona una modalità:',
        'fpv_normal': 'FPV - Rilevamento a pattern (2.442 GHz)',
        'fpv_5g_detection_mode': 'FPV - Rilevamento per anomalie spettrali (5 GHz)',
        'custom_range': 'Intervallo personalizzato per anomalia spettrale',
        'start_mhz': 'Inizio (MHz):',
        'stop_mhz': 'Fine (MHz):',
        'advanced_range': 'Intervalli personalizzati per anomalia spettrale',
        'ok': 'OK',
        'advanced_interval_title': 'Intervallo personalizzato - Intervallo avanzato',
        'up_to_8_intervals': 'Fino a 8 intervalli (MHz)',
        'interval': 'Intervallo {0}:',
        'start': 'Inizio',
        'stop': 'Fine',
        'sweeps': '# scansioni',
        'complete_start_end': 'Completa inizio e fine per l\'intervallo {0}.',
        'invalid_values': 'Valori non validi nell\'intervallo {0}.',
        'end_must_be_greater': 'La fine deve essere maggiore dell\'inizio nell\'intervallo {0}.',
        'sweeps_must_be_positive': 'Il numero di scansioni deve essere maggiore di zero (intervallo {0}).',
        'enter_valid_interval': 'Inserisci almeno un intervallo valido.',
        'enter_numeric_values': 'Inserisci valori numerici validi per inizio e fine.',
        'end_greater_than_start': 'La fine deve essere maggiore dell\'inizio.',
        'language_selection_title': 'Seleziona Lingua',
        'select_language': 'Seleziona una lingua:',
        'could_not_save_language': 'Impossibile salvare la lingua.',
        'tailscale_on': 'TAILSCALE: ON',
        'tailscale_off': 'TAILSCALE: OFF',
        'tailscale_config_title': 'Configurazione Tailscale',
        # 'tailscale_username' y 'tailscale_password' eliminados - no se usan
        'tailscale_not_configured': 'Tailscale non configurato',
        'configure_tailscale_first': 'Configura prima Tailscale usando il pulsante ingranaggio.',
        'tailscale_connecting': 'Connessione a Tailscale...',
        'tailscale_disconnecting': 'Disconnessione da Tailscale...',
        'tailscale_connected': 'Tailscale connesso con successo.',
        'tailscale_disconnected': 'Tailscale disconnesso con successo.',
        'tailscale_error': 'Errore durante la connessione/disconnessione di Tailscale.',
        'tailscale_state_store_error': 'Tailscale non ha potuto inizializzare il suo archivio sicuro (TPM). Apri Tailscale come amministratore e segui la guida ufficiale: https://tailscale.com/s/state-store-init-error . Poi riavvia il servizio Tailscale o reinstalla Tailscale.',
        'tailscale_not_installed': 'Tailscale non è installato. Installalo da https://tailscale.com/download',
        'install_tailscale': 'Installa servizio',
        'installing_tailscale': 'Installazione di Tailscale...',
        'tailscale_install_wait_message': 'Installazione di Tailscale, attendi alcuni secondi.',
        'tailscale_service_installed': 'Servizio Tailscale installato.',
        'tailscale_install_success': 'Tailscale installato con successo. Riavvia l\'applicazione.',
        'tailscale_install_error': 'Errore durante l\'installazione di Tailscale.',
        'tailscale_installer_not_found': 'Installatore Tailscale non trovato nel percorso del progetto.',
        'tailscale_oauth_info': 'Tailscale utilizza l\'autenticazione OAuth (Google, Microsoft, ecc.).\nQuando attivi Tailscale, il tuo browser si aprirà per l\'autenticazione.',
        'tailscale_installed_info': 'Tailscale è installato. Usa il pulsante TAILSCALE: ON/OFF per connetterti.',
        'tailscale_logged_in_as': 'Accesso effettuato come:',
        'tailscale_ip_device': 'IP Tailscale di questo dispositivo:',
        'tailscale_other_devices': 'Altri dispositivi connessi:',
        'tailscale_create_account': 'Crea nuovo account Tailscale',
        'tailscale_sudo_needed': 'Tailscale richiede permessi di amministratore.\n\nEsegui una volta nel terminale:\nsudo tailscale set --operator=$USER\n\nQuindi potrai usare Tailscale senza sudo.',
        'nvidia_cuda_info': 'Si raccomanda, se si dispone di una scheda grafica NVIDIA, di avere i driver aggiornati. Inoltre, su Linux è richiesto NVIDIA CUDA Toolkit per il corretto funzionamento delle librerie (sudo apt install nvidia-cuda-toolkit).',
        'audio_sensitivity_label': 'Sensibilità raw di rilevamento audio tramite Tensorflow (avanzato):',
        'audio_sensitivity_percent': '%',
        'audio_sensitivity_save_error': 'Impossibile salvare la sensibilità audio.',
        'audio_sensitivity_range_error': 'Il valore di sensibilità deve essere compreso tra 1% e 100%.',
        'audio_sensitivity_number_error': 'Il valore di sensibilità deve essere un numero.',
        'rf_flat_loading_message': 'Caricamento rilevamento piatto - Assicurati assenza di droni',
        'rf_flat_loading_progress': 'Calibrazione baseline: {0}/{1}',
        'rf_flat_loading_progress_interval': 'Intervallo {0}/{1}: {2}/{3}',
        'telegram_config_load_error': 'Impossibile caricare la configurazione Telegram.',
        'telegram_config_save_error': 'Impossibile salvare la configurazione Telegram.',
        'telegram_enabled_start': '[TELEGRAM] Notifiche attive.',
        'telegram_disabled_start': '[TELEGRAM] Notifiche disattivate (modifica telegram_config.json per attivarle).',
        'telegram_yolo_message': 'YOLO ha rilevato un drone alle {0}.',
        'telegram_yolo_message_conf': 'YOLO ha rilevato un drone alle {0} (confidenza {1}%).',
        'telegram_rf_message': 'RF ha rilevato un segnale tipo drone alle {0} (confidenza {1}%).',
        'telegram_rf_message_freq': 'RF ha rilevato un segnale tipo drone alle {0}: {1:.3f} MHz ({2}%).',
        'telegram_audio_message': 'Tensorflow ha rilevato un pattern di drone alle {0} (confidenza {1}%).',
        'telegram_audio_size_suffix': 'Firma sonora: {0} ({1}%)',
        'drone_size_small': 'drone piccolo',
        'drone_size_medium': 'drone medio',
        'drone_size_large': 'drone grande',
        'drone_size_inconclusive': 'tipo non conclusivo',
        'telegram_window_title': 'Configurazione bot Telegram',
        'telegram_save_success': 'Configurazione Telegram salvata correttamente.',
        'telegram_section_title': 'Bot Telegram',
        'telegram_setup_help': '1) Apri BotFather e crea il bot.\n2) Incolla qui il token.\n3) Invia /start al bot e premi "Rileva chat_id".',
        'telegram_enable_notifications': 'Attiva notifiche Telegram',
        'telegram_bot_token': 'Token bot:',
        'telegram_chat_id': 'Chat ID:',
        'telegram_detect_chat_id': 'Rileva chat_id',
        'telegram_open_botfather': 'Apri BotFather',
        'telegram_media_options': 'Allegati negli avvisi',
        'telegram_send_yolo_photo': 'Invia foto per rilevamenti YOLO',
        'telegram_send_rf_image': 'Invia immagine spettrogramma RF',
        'telegram_send_audio_clip': 'Invia clip audio per rilevamento sonoro',
        'telegram_cooldowns_label': 'Cooldown per tipo (secondi)',
        'telegram_cooldown_yolo': 'YOLO:',
        'telegram_cooldown_rf': 'RF:',
        'telegram_cooldown_audio': 'Audio:',
        'telegram_cooldown_number_error': 'I cooldown devono essere numeri interi.',
        'telegram_cooldown_range_error': 'I cooldown non possono essere negativi.',
        'telegram_required_fields_error': 'Per attivare Telegram, inserisci token e chat_id.',
        'telegram_chat_id_missing_token': 'Inserisci prima il token del bot.',
        'telegram_chat_id_detect_error': 'Impossibile rilevare automaticamente chat_id.',
        'telegram_chat_id_not_found': 'Nessun messaggio recente. Apri il bot, invia /start e riprova.',
        'telegram_chat_id_detected': 'Chat ID rilevato: {0}',
    },
    'pt': {  # Portuguese
        'yolo_on': 'YOLO: {0} det.',
        'yolo_off': 'YOLO: OFF',
        'det_power_label': 'DET.POWER',
        'det_power_short_low': 'L',
        'det_power_short_med': 'M',
        'det_power_short_high': 'H',
        'det_power_short_vhigh': 'VH',
        'tinysa_on': 'TinySA: ON',
        'tinysa_off': 'TinySA: OFF',
        'det_audio_on': 'DET ÁUDIO: ON',
        'det_audio_off': 'DET ÁUDIO: OFF',
        'audio_source_phone_mic': 'AUDIO: MIC CELULAR',
        'audio_source_esp32_array': 'AUDIO: ARRAY ESP32',
        'ip_label': 'IP: {0}',
        'ip_selector_title': 'Seletor de IP',
        'ip_selector_prompt': 'Selecione um IP disponivel:',
        'ip_selector_no_candidates': 'Nenhum IP candidato encontrado no Tailscale ou rede local.',
        'ip_selector_no_selection': 'Selecione um IP da lista.',
        'ip_selector_current': 'Atual:',
        'ip_selector_adb': 'ADB:',
        'ip_selector_tailscale': 'Tailscale:',
        'ip_selector_tailscale_device': 'Tailscale ({0}):',
        'ip_selector_lan': 'LAN:',
        'fps_label': 'FPS: {0:.1f}',
        'language_app': 'IDIOMA E CONFIG. APP',
        'telegram_notif_app': 'BOT TELEGRAM NOTIF.',
        'ep32_bt_on': 'EP32 BT: ON',
        'ep32_bt_off': 'EP32 BT: OFF',
        'ep32_controls_title': 'Controle EP32',
        'ep32_status_label': 'Estado',
        'ep32_status_unknown': 'desconhecido',
        'ep32_status_off': 'desligado',
        'ep32_status_idle': 'ponte ativa, sem comandos',
        'ep32_status_bridge_unreachable': 'cliente Android nao responde',
        'ep32_status_invalid_url': 'URL do celular invalida',
        'ep32_status_scanning': 'escaneando',
        'ep32_status_connecting': 'conectando',
        'ep32_status_connected': 'conectado',
        'ep32_status_not_connected': 'nao conectado',
        'ep32_status_error': 'erro',
        'ep32_status_invalid_payload': 'payload invalido',
        'ep32_status_method_not_allowed': 'metodo nao permitido',
        'ep32_status_invalid_command': 'comando invalido',
        'ep32_status_invalid_sequence': 'sequencia invalida',
        'ep32_status_invalid_action': 'acao invalida',
        'ep32_btn_up': 'Cima',
        'ep32_btn_down': 'Baixo',
        'ep32_btn_left': 'Esq',
        'ep32_btn_right': 'Dir',
        'ep32_btn_a': 'A',
        'ep32_btn_b': 'B',
        'ep32_btn_menu': 'Menu',
        'ep32_btn_auto': 'Auto',
        'ep32_btn_fix_a': 'Fixar A',
        'ep32_btn_fix_b': 'Fixar B',
        'ep32_btn_auto_a': 'Auto A',
        'ep32_btn_auto_b': 'Auto B',
        'ep32_autotrack_on': 'AUTO-TRACK: ON',
        'ep32_autotrack_off': 'AUTO-TRACK: OFF',
        'no_streaming': 'Streaming não detectado',
        'no_streaming_yolo': 'Não é possível iniciar YOLO porque não há streaming de vídeo disponível.',
        'activate_audio_first': 'Ative primeiro a transmissão de áudio clicando no ícone de volume.',
        'activate_audio_title': 'Ativar transmissão de áudio',
        'no_signal': 'SEM SINAL',
        'reconnecting': 'Tentando reconectar...',
        'audio_drone_detected': 'DRONE DE ÁUDIO DETECTADO: {0}%',
        'no_audio_dron': 'NENHUM DRONE DE ÁUDIO: {0}%',
        'rf_drone_detected': 'DRONE DETECTADO POR RF: {0:.3f} GHz ({1}%)',
        'rf_drone_detected_no_freq': 'DRONE DETECTADO POR RF ({0}%)',
        'tinysa_connected': 'TinySA conectado',
        'tinysa_connected_android': 'TinySA conectado ao Android',
        'adb_connected': 'ADB conectado',
        'tinysa_not_configured': 'TinySA não configurado',
        'configure_tinysa_first': 'Configure TinySA primeiro usando o botão de engrenagem.',
        'tinysa_not_detected': 'TinySA não detectado localmente nem no servidor Android.\nConecte-o via USB ao PC ou ao Android e tente novamente.',
        'change_camera_ip': 'Alterar IP da Câmera',
        'enter_new_ip': 'Digite o novo IP e porta:\n(atual: {0})',
        'yolo_options_title': 'Opções YOLO',
        'available_models': 'Modelos Disponíveis',
        'model': 'Modelo {0}',
        'description': 'Descrição:',
        'browse': 'Procurar',
        'select_yolo_model': 'Selecionar Modelo YOLO',
        'yolo_models': 'Modelos YOLO (*.pt)',
        'all_files': 'Todos os Arquivos',
        'page': 'Página {0}/{1}',
        'load_model': 'Carregar Modelo',
        'load_and_save_default': 'Carregar e Salvar como Padrão',
        'load_default_config': 'Carregar Configuração Padrão',
        'cancel': 'Cancelar',
        'model_updated': 'Modelo atualizado com sucesso.',
        'error': 'Erro',
        'model_empty': 'O modelo no slot {0} está vazio.',
        'file_not_found': 'Arquivo não encontrado:\n{0}',
        'tinysa_mode_selection': 'TinySA - Seleção de Modo',
        'select_mode': 'Selecione um modo:',
        'fpv_normal': 'FPV - Deteccao por padrao (2.442 GHz)',
        'fpv_5g_detection_mode': 'FPV - Deteccao por anomalias espectrais (5 GHz)',
        'custom_range': 'Intervalo personalizado por anomalia espectral',
        'start_mhz': 'Início (MHz):',
        'stop_mhz': 'Fim (MHz):',
        'advanced_range': 'Intervalos personalizados por anomalia espectral',
        'ok': 'OK',
        'advanced_interval_title': 'Intervalo Personalizado - Intervalo Avançado',
        'up_to_8_intervals': 'Até 8 intervalos (MHz)',
        'interval': 'Intervalo {0}:',
        'start': 'Início',
        'stop': 'Fim',
        'sweeps': '# varreduras',
        'complete_start_end': 'Complete início e fim para o intervalo {0}.',
        'invalid_values': 'Valores inválidos no intervalo {0}.',
        'end_must_be_greater': 'O fim deve ser maior que o início no intervalo {0}.',
        'sweeps_must_be_positive': 'O número de varreduras deve ser maior que zero (intervalo {0}).',
        'enter_valid_interval': 'Digite pelo menos um intervalo válido.',
        'enter_numeric_values': 'Digite valores numéricos válidos para início e fim.',
        'end_greater_than_start': 'O fim deve ser maior que o início.',
        'language_selection_title': 'Selecionar Idioma',
        'select_language': 'Selecione um idioma:',
        'could_not_save_language': 'Não foi possível salvar o idioma.',
        'tailscale_on': 'TAILSCALE: ON',
        'tailscale_off': 'TAILSCALE: OFF',
        'tailscale_config_title': 'Configuração Tailscale',
        # 'tailscale_username' y 'tailscale_password' eliminados - no se usan
        'tailscale_not_configured': 'Tailscale não configurado',
        'configure_tailscale_first': 'Configure Tailscale primeiro usando o botão de engrenagem.',
        'tailscale_connecting': 'Conectando ao Tailscale...',
        'tailscale_disconnecting': 'Desconectando do Tailscale...',
        'tailscale_connected': 'Tailscale conectado com sucesso.',
        'tailscale_disconnected': 'Tailscale desconectado com sucesso.',
        'tailscale_error': 'Erro ao conectar/desconectar Tailscale.',
        'tailscale_state_store_error': 'O Tailscale nao conseguiu inicializar seu armazenamento seguro (TPM). Abra o Tailscale como administrador e siga o guia oficial: https://tailscale.com/s/state-store-init-error . Depois reinicie o servico Tailscale ou reinstale o Tailscale.',
        'tailscale_not_installed': 'Tailscale não está instalado. Instale-o em https://tailscale.com/download',
        'install_tailscale': 'Instalar Serviço',
        'installing_tailscale': 'Instalando Tailscale...',
        'tailscale_install_wait_message': 'Instalando Tailscale, aguarde alguns segundos.',
        'tailscale_service_installed': 'Servico Tailscale instalado.',
        'tailscale_install_success': 'Tailscale instalado com sucesso. Reinicie a aplicação.',
        'tailscale_install_error': 'Erro ao instalar Tailscale.',
        'tailscale_installer_not_found': 'Instalador Tailscale não encontrado no caminho do projeto.',
        'tailscale_oauth_info': 'Tailscale usa autenticação OAuth (Google, Microsoft, etc.).\nAo ativar Tailscale, seu navegador será aberto para autenticação.',
        'tailscale_installed_info': 'Tailscale está instalado. Use o botão TAILSCALE: ON/OFF para conectar.',
        'tailscale_logged_in_as': 'Conectado como:',
        'tailscale_ip_device': 'IP Tailscale deste dispositivo:',
        'tailscale_other_devices': 'Outros dispositivos conectados:',
        'tailscale_create_account': 'Criar nova conta Tailscale',
        'tailscale_sudo_needed': 'Tailscale requer permissões de administrador.\n\nExecute uma vez no terminal:\nsudo tailscale set --operator=$USER\n\nDepois poderá usar Tailscale sem sudo.',
        'nvidia_cuda_info': 'Recomenda-se, se você tiver uma placa gráfica NVIDIA, manter os drivers atualizados. Além disso, no Linux, o NVIDIA CUDA Toolkit é necessário para o funcionamento correto das bibliotecas (sudo apt install nvidia-cuda-toolkit).',
        'audio_sensitivity_label': 'Sensibilidade raw de detecção de áudio por Tensorflow (avançado):',
        'audio_sensitivity_percent': '%',
        'audio_sensitivity_save_error': 'Não foi possível salvar a sensibilidade de áudio.',
        'audio_sensitivity_range_error': 'O valor de sensibilidade deve estar entre 1% e 100%.',
        'audio_sensitivity_number_error': 'O valor de sensibilidade deve ser um número.',
        'rf_flat_loading_message': 'Carregando detecção plana - Certifique-se da ausência de drones',
        'rf_flat_loading_progress': 'Calibrando baseline: {0}/{1}',
        'rf_flat_loading_progress_interval': 'Intervalo {0}/{1}: {2}/{3}',
        'telegram_config_load_error': 'Não foi possível carregar a configuração do Telegram.',
        'telegram_config_save_error': 'Não foi possível salvar a configuração do Telegram.',
        'telegram_enabled_start': '[TELEGRAM] Notificações ativas.',
        'telegram_disabled_start': '[TELEGRAM] Notificações desativadas (edite telegram_config.json para ativar).',
        'telegram_yolo_message': 'YOLO detectou um drone às {0}.',
        'telegram_yolo_message_conf': 'YOLO detectou um drone às {0} (confiança {1}%).',
        'telegram_rf_message': 'RF detectou sinal compatível com drone às {0} (confiança {1}%).',
        'telegram_rf_message_freq': 'RF detectou sinal compatível com drone às {0}: {1:.3f} MHz ({2}%).',
        'telegram_audio_message': 'Tensorflow detectou padrão de drone às {0} (confiança {1}%).',
        'telegram_audio_size_suffix': 'Assinatura sonora: {0} ({1}%)',
        'drone_size_small': 'drone pequeno',
        'drone_size_medium': 'drone medio',
        'drone_size_large': 'drone grande',
        'drone_size_inconclusive': 'tipo inconclusivo',
        'telegram_window_title': 'Configuração do bot Telegram',
        'telegram_save_success': 'Configuração do Telegram salva com sucesso.',
        'telegram_section_title': 'Bot do Telegram',
        'telegram_setup_help': '1) Abra o BotFather e crie o bot.\n2) Cole aqui o token.\n3) Envie /start ao seu bot e clique em "Detectar chat_id".',
        'telegram_enable_notifications': 'Ativar notificações por Telegram',
        'telegram_bot_token': 'Token do bot:',
        'telegram_chat_id': 'Chat ID:',
        'telegram_detect_chat_id': 'Detectar chat_id',
        'telegram_open_botfather': 'Abrir BotFather',
        'telegram_media_options': 'Anexos nos alertas',
        'telegram_send_yolo_photo': 'Enviar foto em detecções YOLO',
        'telegram_send_rf_image': 'Enviar imagem de espectrograma RF',
        'telegram_send_audio_clip': 'Enviar trecho de áudio em detecção sonora',
        'telegram_cooldowns_label': 'Cooldown por tipo (segundos)',
        'telegram_cooldown_yolo': 'YOLO:',
        'telegram_cooldown_rf': 'RF:',
        'telegram_cooldown_audio': 'Áudio:',
        'telegram_cooldown_number_error': 'Os cooldowns devem ser números inteiros.',
        'telegram_cooldown_range_error': 'Os cooldowns não podem ser negativos.',
        'telegram_required_fields_error': 'Para ativar Telegram, preencha token e chat_id.',
        'telegram_chat_id_missing_token': 'Informe primeiro o token do bot.',
        'telegram_chat_id_detect_error': 'Não foi possível detectar o chat_id automaticamente.',
        'telegram_chat_id_not_found': 'Sem mensagens recentes. Abra seu bot, envie /start e tente novamente.',
        'telegram_chat_id_detected': 'Chat ID detectado: {0}',
    }
}
